...
function NewJob() {
  ...
  useEffect(() => {
    ...
    RecruiterService.getJobCategories()
      .then((response) => {
        setJobCategories(response.data.map(category => category.name));
      })
      .catch((error) => {
        console.log('Failed to fetch categories:', error);
        // Fallback to hardcoded categories
        setJobCategories(["Java", "React", "Frontend", "Backend"]);
      });
  }, []);
  ...
  const handleSubmit = (e) => {
    ...
    if (jwtToken) {
      ...
      // Add recruiterEmail from JWT token
      const jobWithEmail = {
        ...job,
        recruiterEmail: jwtToken.email || jwtToken.sub
      };
      
      console.log('Job data being sent:', jobWithEmail);
