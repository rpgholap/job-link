...
class RecruiterService {
  ...
  authenticateRecruiter(recruiter) {
    return axios.post(
      "http://localhost:8080/recruiter/authenticate",
      recruiter
    );
  }
  ...
  hireApplicant(jobApplicationId, jwtToken) {
    return axios.get(
      "http://localhost:8080/job-application/hire/" + jobApplicationId,
      {
        headers: {
          Authorization: "Bearer " + jwtToken,
        },
  ...
  updateApplicationStatus(jobApplicationId, status, jwtToken) {
    return axios.get(
      "http://localhost:8080/job-application/update/" +
        status +
        "/" +
        jobApplicationId,
      {
  ...
  uploadCompanyLogo(registeredRecruiterID, formdata) {
    return axios.post(
      `http://localhost:8080/recruiter/image/${registeredRecruiterID}`,
      formdata
    );
  }
  ...
  getJobCategories() {
    return axios.get("http://localhost:8080/job-categories");
  }

  //Admin: Create job category
  createJobCategory(categoryName, jwtToken) {
    return axios.post("http://localhost:8080/admin/job-category", 
      { name: categoryName }, 
      {
        headers: {
          Authorization: "Bearer " + jwtToken,
